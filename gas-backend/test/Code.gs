...
var DEPLOY_MARKER_VERSION = 'v0.6.67';
  ...
  try {
    switch (action) {
      ...
      case 'getPersonnelFromSchedule': {
        ...
        const branchName = (e.parameter.branch || '').toString().trim() || null;
        const result = getPersonnelUnion(yearMonth, startDate, endDate, branchName);
        if (!result.success) {
          return createJsonResponse({ success: false, error: result.error });
        }
        return createJsonResponse({ success: true, names: result.names || [] });
  ...
  try {
    ...
    if (typeof getSingleCompareItem === 'function') {
      item = getSingleCompareItem({
        ...
        scheduleStart: scheduleStart,
        scheduleEnd: scheduleEnd,
        attendanceStart: correctedStart,
        attendanceEnd: correctedEnd
      });
    }
